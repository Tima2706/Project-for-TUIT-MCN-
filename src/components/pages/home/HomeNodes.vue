    ...
    <div v-for="(item, index) in noteList" :key="index" class="mb-2">
      <ACard @click="openFormDialog(item)">
        <div class="flex gap-2 items-center">
          <svg
            data-v-0851bd64="" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"
            fill="none"
          >
            <path
              data-v-0851bd64="" d="M8 4.29333V6.51334" stroke="#13C55E" stroke-miterlimit="10"
            ...
            <path
              ...
              stroke="#13C55E" stroke-miterlimit="10" stroke-linecap="round"
            />
            <path
              data-v-0851bd64=""
              ...
              stroke="#13C55E" stroke-miterlimit="10"
            />
          </svg>
          <VText>{{ item.title }}</VText>
        </div>
        <VText class="flex justify-end pt-2">
          <span
            style="background: rgba(255,215,0,0.84); color: #FFFFFF; border-radius: 6px; font-size: 11px"
            class="p-1"
          >{{ item.deadline ? item.deadline.slice(8, 10) : "" }} {{ $t('day') }} {{ item.deadline ? item.deadline.slice(11, 16) : "" }}</span>
        </VText>
      </ACard>
    </div>
    <a-button
      style="width: 100%; background: rgba(255, 255, 255, 0.60)" class="flex justify-center items-center"
      @click="openFormDialog"
    >
      <svg data-v-0851bd64="" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
        <path
          data-v-0851bd64="" fill-rule="evenodd" clip-rule="evenodd"
